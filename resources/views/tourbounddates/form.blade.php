...
@section('content')
		...
		<div class="box box-primary">
			...
			@if( $row['start']> Carbon::today() or $row['start'] ==NULL )
				<div class="box-body">
					...
					{!! Form::open(array('url'=>'tourbounddates/save?return='.$return, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) !!}
					...
					<div class="form-group  " >
						...
						<div class="col-md-4">
							...
							<select name='tourcategoriesID' rows='5' id='tourcategoriesID' class='select2 ' required>
                                <option>{{ Lang::get('core.pleaseselect') }}</option>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							...
							<button type="button" onclick="location.href='{{ URL::to('tourbounddates?return='.$return) }}' " class="btn btn-danger btn-sm ">  {{ Lang::get('core.sb_cancel') }} </button>
						</div>
	...
	<script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#tourID").jCombo("{!! url('tourbounddates/comboselect?filter=tours:tourID:tour_name&limit=WHERE:departs:=:3') !!}&parent=tourcategoriesID:",
                {  parent: '#tourcategoriesID', selected_value : '{{ $row["tourID"] }}' });

            $("#guideID").jCombo("{!! url('tourbounddates/comboselect?filter=guides:guideID:name&limit=WHERE:status:=:1') !!}",
                {  selected_value : '{{ $row["guideID"] }}' });

            $("#currencyID").jCombo("{!! url('tourbounddates/comboselect?filter=def_currency:currencyID:currency_sym|symbol&limit=WHERE:status:=:1') !!}",
                {  selected_value : '{{ $row["currencyID"] }}' });


            $('.removeMultiFiles').on('click',function(){
                var removeUrl = '{{ url("tourbounddates/removefiles?file=")}}'+$(this).attr('url');
                $(this).parent().remove();
        ...
        $("input[name='total_capacity'], input[name='cost_single'], input[name='cost_double'], input[name='cost_triple'], input[name='cost_quad'], input[name='cost_quint'], input[name='cost_sext'], input[name='cost_child'], input[name='cost_child_wo_bed'], input[name='cost_infant_wo_bed'], input[name='cost_deposit']").TouchSpin();
	</script>
