            ...
            @foreach ($rowData as $row)
                <tr>
					...
					<td>
							...
							<a href="{{ url('visaapplication/show/'.$row->applicationID.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-eye fa-2x"></i> </a>
							@endif
							@if($access['is_edit'] ==1)
							<a  href="{{ url('visaapplication/update/'.$row->applicationID.'?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-pencil fa-2x"></i> </a>
							@endif
