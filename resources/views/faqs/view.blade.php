...
<section class="content-header">
  <h1>{{ Lang::get('core.faq') }}</h1>
    ...
    <div class="box-header-tools pull-left" >
				...
				@if($access['is_add'] ==1)
		   		<a href="{{ URL::to('faqs/update/'.$id.'?return='.$return) }}" class="tips text-green" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-pencil fa-2x"></i></a>
        ...
        <div class="box-body">
			...
			@foreach($faqTree as $fs)
				...
				<li><h4>{{ $fs['title'] }}
					...
					@endif
					@if($access['is_edit'] =='1')
					<a href="{{ url('faqs/section/'.$row->faqID.'/'.$fs['sectionID'])}}"  onclick="MmbModal(this.href,'{{ Lang::get('core.editsection') }}'); return false;"  class="tips text-blue  pull-right" title="{{ Lang::get('core.editsection') }}"><i class="fa fa-pencil fa-lg"></i></a>
					@endif	</h4> 				
          ...
          <div class="box-body">
			@if(isset($items) && count($items)>=1)
				<?php $items = $items[0]; ?>
					...
					<a href="{{ url('faqs/item/'.$row->faqID.'/'.$item->sectionID.'/'.$items->id)}}" onclick="MmbModal(this.href,'{{ Lang::get('core.editquestion') }}'); return false;" class="tips text-blue" title="{{ Lang::get('core.editfaq') }}"><i class="fa fa-pencil fa-2x"></i></a>
