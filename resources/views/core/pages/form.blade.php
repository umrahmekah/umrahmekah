...
<div class="content">
		 ...
		 {!! Form::open(array('url'=>'core/pages/save/'.$row['pageID'], 'class'=>'form-vertical row ','files' => true , 'data-parsley-validate'=>'','novalidate'=>' ')) !!}			
						...
						{!! Form::text('alias', $row['alias'],array('class'=>'form-control', 'placeholder'=>'', 'style'=>'width:150px; display:inline-block;'   )) !!} @if ($row['alias']!=NULL) <a href="{{ URL::to($row['alias'])}}" class="tips btn btn-sm btn-primary" target="_blank" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-eye fa-lg"></i></a>@endif
							  ...
							  <div class="form-group  " >
								<div class="" style="background:#fff;">
								  <textarea name='note' rows='10' id='note'    class='form-control editor'  
									 >{{ $row['note'] }}</textarea> 
								 </div> 
							  </div> 						  
            ...
            <div class="box-header with-border">
              <h3 class="box-title">{{ Lang::get('core.page_setting') }}</h3>
              ...
              <div class="form-group  " >
                ...
                <div class="form-group  " >
					...
					<label class="checkbox"><input  type='checkbox' name='allow_guest' 
 						@if($row['allow_guest'] ==1 || $row['pageID'] == "") checked  @endif	
					   value="1"	/> </lable>
				  </div>
				  <div class="form-group  " >
					...
					<label class="radio">					
					  ...
					  @if( $row['status'] =='enable'  || $row['pageID'] == "")  	checked	  @endif				  
					   /> 
				  ...
				  <div class="form-group  " >
					...
					<label class="radio">					
					  ...
					  @if( $row['template'] =='frontend'  || $row['pageID'] == "")  	checked	  @endif				  
					   /> 
				  ...
				  <div class="form-group  " >
					<label for="header" > {{ Lang::get('core.banners') }} </label>
					<select class="form-control" name="header">
						<option value="" @if(empty($row['header'])) selected @endif>{{ Lang::get('core.no_banners') }}</option>
						<option value="home" @if($row['header'] == 'home') selected @endif>home</option>
						@foreach($banner_position as $val)
							<option value="{{ preg_replace('/\s+/', '', $val->position_name) }}" @if($row['header'] == preg_replace('/\s+/', '', $val->position_name) ) selected @endif>{{ $val->position_name}}</option>
						@endforeach
					</select>
				  </div>
            </div>
