            ...
            @foreach ($rowData as $row)
                <tr>
					...
					<td>
							...
							<a href="{{ URL::to('core/users/show/'.$row->id.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-eye fa-2x"></i></a>
							@endif
							@if($access['is_edit'] ==1)
							<a  href="{{ URL::to('core/users/update/'.$row->id.'?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-pencil fa-2x"></i></a>
							@endif

					</td>
                    <td>	<?php if( file_exists( './uploads/users/'.$row->avatar) && $row->avatar !='') { ?>
							<img src="{{ URL::to('uploads/users').'/'.$row->avatar }} " border="0" width="40" height="40" class="img-circle" />
							<?php  } else { ?> 
							<img alt="" src="https://www.gravatar.com/avatar/{{ md5($row->email) }}" width="40" height="40"  class="img-circle" />
							<?php } ?>					 				 
                    </td>
                    <td>{{ $row->name}}</td>
                    <td>{{ $row->username}}</td>
