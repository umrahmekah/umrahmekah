			...
			<div class="box-body">
				...
				<div class="col-md-12">
					...
					<div class="form-group  " >
						...
						<div class="col-md-6">
							<input  type='text' name='tour_name' id='tour_name' value='{{ old('tour_name',$row['tour_name']) }}' class='form-control' maxlength="40" required />
							<span class="help-block m-b-none">Up to 40 characters only.</span><span style="color: red;">@if($name_error ?? false) The name is already used in other tour. Please change the name @endif</span>
						</div>
						<div class="col-md-2">
						</div>
					</div>
						...
						<div class="col-md-3">
							...
							<select name='tourcategoriesID' rows='3' id='tourcategoriesID' class='select2 ' required>
								@foreach($categories as $cat)
									<option value="{{ old('tourcategoriesID',$cat->tourcategoriesID) }}" @if ($cat->tourcategoriesID == $row['tourcategoriesID'])
										selected
									@endif>{{ $cat->tourcategoryname }}</option>
								@endforeach
							</select>
						</div>
						<div class="col-md-3">
					...
					<div class="form-group  " >
						...
						<div class="col-md-7">
							...
							<label class='checked checkbox-inline'>
								<input type='checkbox' name='featured[]' id="featured" value ='1'   class=''
					...
					<div class="form-group  " >
						<label for="B2B" class=" control-label col-md-4 text-left"> {{ Lang::get('core.b2b') }} </label>
						<div class="col-md-7">
							<label class='checked checkbox-inline'>
								<input type='checkbox' name='b2b' id="b2b" value ='1'   class=''
									   @if($row['b2b'] == 1)checked @endif
								/>  </label>
						</div>
					</div>
					...
					<div class="form-group tourduration" >
						...
						<div class="col-md-2 ">
							...
							<input  type='text' name='total_days' id='total_days' value='{{ old('total_days',$row['total_days']) }}' required class='form-control ' />
						</div>
						<div class="col-md-2">
							<label class="control-label col-md-1- text-right">{{ Lang::get('core.nights') }}</label>
							<input  type='text' name='total_nights' id='total_nights' value='{{ old('total_nights',$row['total_nights']) }}'
									required     class='form-control ' />
						</div>
					</div>
					<div class="form-group  " >
						<label for="Flight" class=" control-label col-md-4 text-left"> {{Lang::get('core.flight')}} </label>
						<div class="col-md-6">
							<select name='flight' rows='3' id='flight' class='select2 '>
								<option>Choose your Air Line</option>
								@foreach(DB::table('flights')->get() as $flight)
									<option value="{{old('iata',$flight->iata)}}">{{$flight->name}}</option>
								@endforeach
							</select>
						</div>
						<div class="col-md-3">
				...
				<div class="form-group  " >
					...
					<div class="col-md-6">
						<input  type='text' name='transit' id='transit' value='{{ old('transit',$row['transit']) }}' class='form-control ' />
					</div>
					<div class="col-md-2">
					</div>
				</div>
				<div class="form-group  " >
					...
					<div class="col-md-6">
						<input  type='text' name='sector' id='sector' value='{{ old('sector',$row['sector']) }}' class='form-control ' />
					</div>
					<div class="col-md-2">
					</div>
				</div>
				<div class="form-group" >
					...
					<div class="col-md-5">
						<div class="col-md-4 baggage-form">
							<input  type='number' name='baggage_limit' id='baggage_limit' value='{{ old('baggage_limit',$row['baggage_limit']) }}' class='form-control ' />
						</div>
						<div class="col-md-1">
				...
				<div class="form-group  " >
					<label for="Landing Page Image" class=" control-label col-md-4 text-left"> {{ Lang::get('core.landingimage') }} <span class="asterix"> * </span></label>
					<div class="col-md-6">
						<div class="btn btn-primary btn-file"><i class="fa fa-camera fa-lg"></i>
							<input  type='file' name='landing_image' id='landing_image' @if($row['landing_image'] =='') class='required' @endif style='width:150px !important;'  />
						</div>
						<div >
							{!! SiteHelpers::showUploadedFile($row['landing_image'],'/uploads/images/'.CNF_OWNER.'/') !!}
						</div>
				...
				<div class="form-group  " >
					<label for="View Page Image" class=" control-label col-md-4 text-left"> {{ Lang::get('core.viewimage') }} <span class="asterix"> * </span></label>
					<div class="col-md-6">
						<div class="btn btn-primary btn-file"><i class="fa fa-camera fa-lg"></i>
							<input  type='file' name='view_image' id='view_image' @if($row['view_image'] =='') class='required' @endif style='width:150px !important;'  />
						</div>
						<div >
							{!! SiteHelpers::showUploadedFile($row['view_image'],'/uploads/images/'.CNF_OWNER.'/') !!}
						</div>
	...
	<script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#countryID").jCombo("{!! url('tourdetail/comboselect?filter=def_country:countryID:country_name&limit=WHERE:countryID:=:184') !!}",
                {  selected_value : '{{ old("countryID",$row["countryID"]) }}' });

            $("#inclusions").jCombo("{!! url('tours/comboselect?filter=def_inclusions:inclusionID:inclusion&limit=WHERE:owner_id:=:'.CNF_OWNER.'&limit=WHERE:type:=:1') !!}",
            ...
            $("#policyandterms").jCombo("{!! url('tours/comboselect?filter=termsandconditions:tandcID:title&limit=WHERE:owner_id:=:'.CNF_OWNER.'&limit=WHERE:type:=:1') !!}",
                {  selected_value : '{{ old("policyandterms",$row["policyandterms"]) }}' });
