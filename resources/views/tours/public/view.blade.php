...
@endif	
          ...
          <h2 class="text-uppercase text-letter-spacing-l my-0 text-inverse font-weight-bold">
            <img class="rounded img-fluid float-left mx-3 my-3" src="@if(file_exists(public_path().'/uploads/images/'.CNF_OWNER.'/'.$row->tourimage) && $row->tourimage !='')
        ...
        @foreach($similartours as $tour)
          <div class="card product-card overlay-hover">
            ...
            <div class="pos-relative">
              <img class="card-img-top img-fluid" src="uploads/images/<?php echo CNF_OWNER;?>/{{ SiteHelpers::formatLookUp($tour,'tourID','1:tours:tourID:tourimage')}}" alt="{{ SiteHelpers::formatLookUp($tour,'tourID','1:tours:tourID:tour_name')}}">
            </div>
          </div>
          @endforeach
        </div>
