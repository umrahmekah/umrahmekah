  ...
  <div class="content"> 
	...
	<div class="box-body"> 	
		<ul class="parsley-error-list">
		 ...
		 {!! Form::open(array('url'=>'tourboundbooking/save?return='.$return, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) !!}
...
<!-- <div class="col-md-12">
    ...
    @endif
			...
			<div style="clear:both"></div>	
				  <div class="form-group">
					...
					<button type="button" onclick="location.href='{{ URL::to('tourboundbooking?return='.$return) }}' " class="btn btn-danger"> {{ Lang::get('core.sb_cancel') }} </button>
					</div>	  
				  </div> 
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#travellerID").jCombo("{!! url('tourboundbooking/comboselect?filter=travellers:travellerID:nameandsurname&limit=WHERE:status:=:1&limit=WHERE:owner_id:=:'.CNF_OWNER) !!}",
        {  selected_value : '@if ( app('request')->input('travellerID') != NULL ) {{app('request')->input('travellerID')}} @else {{ $row["travellerID"] }} @endif' });

		 
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("tourboundbooking/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
