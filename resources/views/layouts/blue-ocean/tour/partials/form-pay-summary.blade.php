<div class="payment__container">
    <div class="payment__summary">
        ...
        <!-- package summary -->
        <h5 class="text-uppercase mb-4">{{ Lang::get('core.tourdetails') }}</h5>
        <div class="details-desc">
            <p><span class="text-default"><i class="fas fa-tree mr-4"></i>Umrah Name: </span>{{ $detail['tour_name'] }}</p>
            <p><span class="text-default"><i class="fas fa-tags mr-3"></i>{{ Lang::get('core.category') }}: </span>{{ $category->tourcategoryname }}</p>
            <p><span class="text-default"><i class="far fa-calendar-alt mr-4"></i>{{ Lang::get('core.date') }}: </span>{{ $tourdatedetail['start'] }} TO {{$tourdatedetail['end'] }}</p>
            <p><span class="text-default"><i class="far fa-clock mr-4"></i>{{ Lang::get('core.duration') }}: </span>
            {!! Lang::get('core.days_night', [
                'totaldays' => $detail['total_days'],
                'totalnight' => $detail['total_nights']
            ]) !!}
            {{-- {{ $detail['total_days'] }} DAYS AND {{$detail['total_nights'] }} NIGHT</p> --}}
            <p><span class="text-default"><i class="fas fa-shipping-fast mr-3"></i>Transit: </span>{{ $detail['transit'] }}</p>
            <p><span class="text-default"><i class="fas fa-briefcase mr-4"></i>{{ Lang::get('core.baggage_limit') }}: </span>{{ $detail['baggage_limit']}}KG</p>
    ...
    <div class="payment__price payment__detail hidden" style="background: white;">
        ...
        <!-- payment summary -->
        <h5 class="text-uppercase mb-4">{{ Lang::get('core.paymentdetails') }}</h5>
        <div>
            <p>
                <span class="text-default"><i class="fas fa-users mr-3"></i>{{ Lang::get('core.number_of_jemaah') }}: </span>
                <span class="font-weight-bold text-default" id="total_jemaah1">                    
            </p>
            <p>
                <span class="text-default"><i class="far fa-money-bill-alt mr-3"></i>{{ Lang::get('core.deposit_per_jemaah') }}: </span>
                <span class="font-weight-bold text-default">{{CURRENCY_SYMBOLS}}{{$tourdatedetail['cost_deposit'] }}
            </p>
            <p>
                <span class="text-default"><i class="fas fa-money-bill-alt mr-3"></i>{{ Lang::get('core.totaldeposit') }}: </span>
                <span class="font-weight-bold text-default" id="total_deposit1">
            </p>
        </div>
        <div class="payment__price-total xy-center">
            <span><i class="fas fa-tag"></i>{{ Lang::get('core.total_price') }}</span>
            <span class="font-weight-bold font-medium text-dark">
                <span class="text-default" id="total_price1"></span>
            </span>
        </div>
        <input type="hidden" name="balance" id="balance">
    </div>
    <button type="button" class="payment__button payment__detail hidden btn btn-primary btn-block" {{-- onclick="submitButton();" --}} onclick="toggleTerms();">
        <span>@if(CNF_BILLPLZAPIKEY) {{ Lang::get('core.book_and_pay_deposit') }} @else {{ Lang::get('core.book_and_pay_later') }} @endif</span>
    </button>
</div>
            ...
            <div class="modal-body">
                <div class="container py-4">
                    <div class="col-xs-12 col-sm-12 p-4">
                        <form class="mb-3">
                            <h3 class="font font-normal font-weight-bold text-black3">{{ Lang::get('core.login_youhaveaccount') }}</h3>
                            ...
                            <button type="button" class="btn btn-blue" onclick="proceedSignIn();">{{ Lang::get('core.signin') }}!</button>
                        </form>
                    </div>
                </div>
            </div>
