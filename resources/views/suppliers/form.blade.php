		...
		<div class="box-body">
			...
			<div class="col-md-12">
				<fieldset><legend> {{ Lang::get('core.suppliers') }}</legend>
					...
					<div class="form-group  " >
						...
						<div class="col-md-6">
							...
							<div class="input-group m-b" style="width:150px !important;">
								{!! Form::date('start_date', $row['start_date'] ?? null,array('class'=>'form-control input-sm date')) !!}
					...
					<div class="form-group  " >
						<label for="Expired Date" class=" control-label col-md-4 text-left">{{Lang::get('core.expdate')}}</label>
						<div class="col-md-6">

							<div class="input-group m-b" style="width:150px !important;">
								{!! Form::date('expired_date', $row['expired_date'] ?? null,array('class'=>'form-control input-sm date')) !!}
								<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
							</div>
						</div>
						<div class="col-md-2">
				...
				<fieldset>
					<legend> {{ Lang::get('core.document') }}</legend>
					<div class="form-group  " >
						<label for="Document" class=" control-label col-md-4 text-left">{{Lang::get('core.document')}}</label>
						<div class="col-md-6">
							<div class="row">
								<div class="col-md-2">
									<a href="javascript:void(0)" class="btn btn-xs btn-primary pull-right" onclick="addMoreFiles('document')"><i class="fa fa-plus"></i></a>
								</div>
								<div class="col-md-10">
									<div class="documentUpl">
					...
					<div class="form-group">
						<div class="col-md-12">
							<ul class="uploadedLists " >
                                <?php $cr= 0;
                                $row['document'] = explode(",",$row['document'] ?? null);
                                ?>
								@foreach($row['document'] as $files)
									@if(file_exists('.'.$row['owner_id'].'/'.$files) && $files !='')
										<li id="cr-<?php echo $cr;?>" class="">
											<a href="{{ url('.'.$row['owner_id'].'/'.$files) }}" target="_blank" >{{ $files }}</a>
											<span class="pull-right removeMultiFiles" rel="cr-<?php echo $cr;?>" url="{{$files}}">
											<i class="fa fa-trash-o  btn btn-xs btn-danger"></i></span>
											<input type="text" name="currdocument[]" value="{{ $files }}"/>
                                            <?php ++$cr;?>
										</li>
									@endif
