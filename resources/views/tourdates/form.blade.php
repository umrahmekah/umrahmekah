...
@section('content')
				...
				<div class="box-body">
					...
					<div class="form-group  " >
						...
						<div class="col-md-4">
							...
							<select name='tourcategoriesID' rows='5' id='tourcategoriesID' class='select2 ' required onchange="changeCategory(this.value);">
                                <option>--Please Select--</option>
								@foreach($categories as $cat)
									<option value="{{ $cat->tourcategoriesID }}" @if ($tourdate->tourcategoriesID == $cat->tourcategoriesID || ($tours&&$tours->tourcategoriesID == $cat->tourcategoriesID))
										selected="selected" 
					...
					<div class="form-group  " >
						...
						<div class="col-md-4">
							<select name='tourID' rows='5' id='tourID' class='select2 ' required  >
							</select>
						</div>
					</div>
					<div class="form-group  " >
						...
						<div class="col-md-7">
							...
							<label class='checked checkbox-inline'>
								<input type='checkbox' name='featured[]' value ='1'   class=''
									   @if($row['featured'] == 1) checked @endif
								/>  </label>
						</div>
					</div>
					...
					<fieldset><legend>{{ Lang::get('core.tourfeatures') }}</legend>
						...
						<div class="form-group  " >
							<label for="Tour Cost for a Child" class=" control-label col-md-5 text-left">{{ Lang::get('core.min_deposit')}}</label>
							<div class="col-md-2">
								<input  type='text' name='cost_deposit' id='cost_deposit' value='{{ $row['cost_deposit'] }}'
										class='form-control ' />
							</div>
						</div>
						...
						<div class="form-group  " >
							<label for="Tour Cost for Quad Room" class=" control-label col-md-5 text-left"> {{ Lang::get('core.tourcostquad') }} </label>
							<div class="col-md-2">
						...
						<div class="form-group  " >
							<label for="Tour Cost for Quint Room" class=" control-label col-md-5 text-left"> {{ Lang::get('core.tourcostquint') }} </label>
							<div class="col-md-2">
								<input  type='text' name='cost_quint' id='cost_quint' value='{{ $row['cost_quint'] }}'
										class='form-control ' />
							</div>
						</div>
						<div class="form-group  " >
							<label for="Tour Cost for Sext Room" class=" control-label col-md-5 text-left"> {{ Lang::get('core.tourcostsext') }} </label>
							<div class="col-md-2">
								<input  type='text' name='cost_sext' id='cost_sext' value='{{ $row['cost_sext'] }}'
										class='form-control ' />
							</div>
						</div>
						<div class="form-group  " >
						...
						<div class="form-group  " >
							<label for="Tour Cost for a Child without bed" class=" control-label col-md-5 text-left"> {{ Lang::get('core.tourcostchildwithoutbed') }} </label>
							<div class="col-md-2">
								<input  type='text' name='cost_child_wo_bed' id='cost_child_wo_bed' value='{{ $row['cost_child_wo_bed'] }}'
										class='form-control ' />
							</div>
						</div>
						<div class="form-group  " >
							<label for="Tour Cost for a Infant without bed" class=" control-label col-md-5 text-left"> {{ Lang::get('core.tourcostinfant') }} </label>
							<div class="col-md-2">
								<input  type='text' name='cost_infant_wo_bed' id='cost_infant_wo_bed' value='{{ $row['cost_infant_wo_bed'] }}'
										class='form-control ' />
							</div>
						</div>
                        
                        <div class="form-group " style="margin-top:3%" >
							<label for="Discount" class=" control-label col-md-5 text-left"> Discount </label>
							<div class="col-md-2">
								<input  type='text' name='discount' id='discount' value='{{$row['discount']}}'
										class='form-control ' />
							</div>
						</div>
	...
	<script type="text/javascript">
        ...
        $("input[name='total_capacity'], input[name='cost_single'], input[name='cost_double'], input[name='cost_triple'], input[name='cost_quad'], input[name='cost_quint'], input[name='cost_sext'], input[name='cost_child'], input[name='cost_child_wo_bed'], input[name='cost_infant_wo_bed'],input[name='discount'], input[name='cost_deposit']").TouchSpin();
        
        
        
        function changeCategory(id, select_id = null) {
        	axios.post('/tourdates/tours', {id:id})
        	.then(response=>{
        		let data = response.data;
        		const tour = document.getElementById('tourID');
        		let string = "<option hidden selected>--Please select--</option>";
        		for(i in data){
        			if (select_id == data[i]['tourID']) {
        				string += "<option selected value=\""+data[i]['tourID']+"\">"+data[i]['tour_name']+"</option>";
        			}else{
        				string += "<option value=\""+data[i]['tourID']+"\">"+data[i]['tour_name']+"</option>";
        			}
        		}
        		tour.innerHTML = string;
        	}).catch(e=>{
        ...
        @if ($row["tourID"])
        	changeCategory({{ $tourdate->tourcategoriesID }}, {{ $row["tourID"] }})
        @endif
        
        
        @if ($tour_id)
            let categoryID = document.getElementById("tourcategoriesID").value;
        	changeCategory(categoryID, {{ $tour_id }});
        @endif
	</script>
