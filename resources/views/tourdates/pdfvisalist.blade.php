...
<body>
	...
	<table style="border-style: none">
		<tr>
			<td style="border-style: none; width: 150px;"><b>{{ Lang::get('core.masterlistfor') }}</b></td>
					...
					@foreach($booking->bookRoom as $room)
						@foreach($room->travellerList as $traveller)
							<tr>
								...
								@endif
								@if(!$traveller->gender && $traveller->age > 12)
									<td style="color: red">{{ Lang::get('core.sexnotfound') }}</td>
								@elseif($traveller->gender === "M" && $traveller->age > 12)
									<td>{{ Lang::get('core.mahram_not_required') }}</td>
								@elseif($traveller->mahram)
									<td>{{ $traveller->mahramRelationship }}: {{ $traveller->mahram->fullname }}</td>
								@else
									<td style="color: red">{{ Lang::get('core.mahram_not_found') }}</td>
								@endif
							</tr>
						@endforeach
					@endforeach
