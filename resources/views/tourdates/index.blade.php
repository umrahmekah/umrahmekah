...
<div class="box box-primary">
    ...
    <table class="table table-striped table-bordered " id="{{ $pageModule }}Table">
        <thead>
			<tr>
				<th class="number" style="vertical-align: top"> No </th>
				<th style="vertical-align: top"> <input type="checkbox" class="checkall" /></th>
				<!-- <th >{{ Lang::get('core.capacity') }}</th> -->
				<!-- <th >{{ Lang::get('core.tourcategory') }}</th> -->
                <th style="vertical-align: top" >{{ Lang::get('core.tourcodes') }}</th>
                <th style="vertical-align: top">{{ Lang::get('core.tourname') }}</th>
                <th style="vertical-align: top">{{Lang::get('core.payment_status')}}</th>
                <th style="vertical-align: top">{{Lang::get('core.visa_status')}}</th>
				<!-- <th >{{ Lang::get('core.tourcode') }}</th> -->
				<th style="vertical-align: top">{{ Lang::get('core.departuredates') }}</th>
				<!-- <th >{{ Lang::get('core.end') }}</th> -->
				<!-- <th >{{ Lang::get('core.guide') }}</th> -->
                <th style="vertical-align: top">{{ Lang::get('core.note') }}</th>
                <th style="vertical-align: top">{{ Lang::get('core.btn_action') }}</th>
			  </tr>
        </thead>
        ...
        <tbody>
            @foreach ($rowData as $row)
                ...
                <tr>
                    ...
                    <td>
                    @if($today>=$row->end)
                        <strike> @if($row->tourdate->tour) {{ $row->tour_code }} @else Umrah Package Not Found @endif </strike>
                    @else   
                        @if($row->tourdate->tour) {{ $row->tour_code }} @else Umrah Package Not Found @endif 
                    @endif
                    </td>
                    ...
                    <td>
                    @if($today>=$row->end)
                        <strike> @if($row->tourdate->tour) {{ $row->tourdate->tour->tour_name }} @else Umrah Package Not Found @endif </strike>
                    @else   
                        @if($row->tourdate->tour) {{ $row->tourdate->tour->tour_name }} @else Umrah Package Not Found @endif 
                    @endif
                    </td>
                    <td>{{$paid[$i-1]}}</td>
                    <td>TBA</td>
                    ...
                    <td>
                        @if($today>=$row->end)
                            ...
                            <strike>{{ \Carbon\Carbon::parse($row->start)->format('d/m/Y')}}</strike>
                        @else
                            {{ \Carbon\Carbon::parse($row->start)->format('d/m/Y')}}
                        @endif
                    </td>
                    ...
                    <td>@if($row->tourdate->existBookingNeedsAttention)<i class="text-red fa fa-exclamation-triangle fa-2x tips" title="This Booking Needs Attention"></i>@endif</td>

                    <td style="width: 190px;">
                        @if($access['is_detail'] ==1)
                            <a href="{{ url('tourdates/show/'.$row->tourdateID.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-eye fa-2x"></i> </a>
                            @endif
                            @if($access['is_edit'] ==1)
                            <a   class="tips"
                            @if( $row->start < $today)
                        ...
                        {!! GeneralStatus::Tour($row->status,$row->start,$row->end,$row->tourdateID, $row->total_capacity, $row->tourdate->pax) !!}
                    </td>
                </tr>
