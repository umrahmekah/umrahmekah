            ...
            @foreach ($rowData as $row)
                <tr>
					...
					<td>
							...
							<a href="{{ url('sites/show/'.$row->siteID.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-eye fa-2x"></i> </a>
							@endif
							@if($access['is_edit'] ==1)
							<a  href="{{ url('sites/update/'.$row->siteID.'?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-pencil fa-2x"></i> </a>
							@endif
					</td>
                    <td>{{ $row->site_name}}</td>
