...
class TablesController extends Controller
{
    ...
    public function getTableconfig($table = null)
    {
        ...
        $this->data['tbtypes'] = ['bigint', 'binary', 'bit', 'blob', 'bool', 'boolean', 'char', 'date', 'datetime', 'decimal', 'double', 'enum', 'float', 'int', 'longblob', 'longtext', 'mediumblob', 'mediuminit', 'mediumtext', 'numerice', 'real', 'set', 'smallint', 'text', 'time', 'timestamp', 'tinyblob', 'tinyint', 'tinytext', 'varbinary', 'varchar', 'year'];

        $this->data['engine'] = ['MyISAM', 'InnoDB'];
        $this->data['info']   = $info;

        $this->data['columns'] = $columns;
        $this->data['table']   = $table;
        $this->data['action']  = (null == $table ? 'mmb/tables/tables/' . $table : 'mmb/tables/tableinfo/' . $table);

        return view('mmb.tables.config', $this->data);
    }
    ...
    public function postTableremove(Request $request)
    {
        ...
        if (! is_null($request->input('id')) && count($request->input('id')) >= 1) {
            ...
            return Redirect::to('mmb/tables')->with('messagetext', 'Table(s) has been deleted')->with('msgstatus', 'success');
        }

        return Redirect::to('mmb/tables')->with('messagetext', 'error', 'No Table(s) deleted !')->with('msgstatus', 'error');
    }
    ...
    public function getTablefieldedit($table)
    {
        ...
        return view('mmb.tables.field', $this->data);
    }
    ...
    public static function lenght($type, $lenght)
    {
        if ('' == $lenght) {
