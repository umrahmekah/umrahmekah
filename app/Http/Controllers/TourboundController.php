...
class TourboundController extends Controller
{
    public $module          = 'tourbound';
    ...
    public function getIndex(Request $request)
    {
        ...
        return view('tourbound.index', $this->data);
    }
    ...
    public function getUpdate(Request $request, $id = null)
    {
        ...
        $categories               = Tourboundcategory::where('owner_id', CNF_OWNER)->where('type', 2)->get();
        $this->data['categories'] = $categories;
        $this->data['fields']     = \SiteHelpers::fieldLang($this->info['config']['forms']);

        $this->data['id'] = $id;

        return view('tourbound.form', $this->data);
    }
    ...
    public function getShow(Request $request, $id = null)
    {
        ...
        if ($row) {
            ...
            return view('tourbound.view', $this->data);
        } else {
            return Redirect::to('tourbound')->with('messagetext', 'Record Not Found !')->with('msgstatus', 'error');
        }
    }
    ...
    public function postCopy(Request $request)
    {
        ...
        if (count($request->input('ids')) >= 1) {
            ...
            return Redirect::to('tourbound')->with('messagetext', \Lang::get('core.note_success'))->with('msgstatus', 'success');
        } else {
            return Redirect::to('tourbound')->with('messagetext', 'Please select row to copy')->with('msgstatus', 'error');
        }
    }
    ...
    public function postSave(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if (! is_null($request->input('apply'))) {
                $return = 'tourbound/update/' . $id . '?return=' . self::returnUrl();
            } else {
                $return = 'tourbound?return=' . self::returnUrl();
            }
            ...
            return Redirect::to('tourbound/update/' . $request->input('tourID'))->with('messagetext', \Lang::get('core.note_error'))->with('msgstatus', 'error')
    ...
    public function display(Request $request)
    {
        ...
        $category  = \DB::table('def_tour_categories')
            ...
            ->where('def_tour_categories.type', 2)
        ...
        if ('view' == $mode) {
            ...
            $tourdate   = \DB::table('tour_date')->select(DB::raw('tour_date.*,tours.tour_name,tours.transit'))
                ...
                ->where('tour_date.tourID', $id)->where('tour_date.status', '1')
                ->orderBy('tour_date.start', 'ASC')
                ->get();

            $tourdate = Tourbounddates::where('tourID', $_GET['view'])->where('status', 1)->get()->filter( function ($query) {
                return $query->notStartYet && $query->capacity > 0;
            });

            //find min price for each room
            $minsingle = Tourbounddates::select('cost_single')->where('tourID', $id)->where('cost_single', '!=', 0)->min('cost_single');
            $mindouble = Tourbounddates::select('cost_double')->where('tourID', $id)->where('cost_double', '!=', 0)->min('cost_double');
            $mintriple = Tourbounddates::select('cost_triple')->where('tourID', $id)->where('cost_triple', '!=', 0)->min('cost_triple');
            $minquad   = Tourbounddates::select('cost_quad')->where('tourID', $id)->where('cost_quad', '!=', 0)->min('cost_quad');
            $minquint   = Tourbounddates::select('cost_quint')->where('tourID', $id)->where('cost_quint', '!=', 0)->min('cost_quint');
            $minsext   = Tourbounddates::select('cost_sext')->where('tourID', $id)->where('cost_sext', '!=', 0)->min('cost_sext');
            ...
            if ($row) {
                ...
                $data['row']       = Tourbound::find($row->tourID);
                $data['fields']    = \SiteHelpers::fieldLang($info['config']['grid']);
                $data['id']        = $id;
                $data['category']  = Tourboundcategory::where('tourcategoriesID', $row->tourcategoriesID)->where('type', 2)->first();

                $data['seat_available_total'] = 0;
                // $data['category']  = Tourboundcategory::where('tourcategoriesID', $row->tourcategoriesID)->first();
            ...
            $tours = Tourbound::where('owner_id', CNF_OWNER)->where('type', 2)->where('status', 1);       
         
            if ($request->cat) {        
                $tours = $tours->where('tourcategoriesID', $request->cat);      
            }       
        
            $tours = $tours->orderBy($sort, $order)->get();
            ...
            $tourdates = Tourbounddates::select('tourID', 'start', 'end')->where('status', 1)->where('start', '>', \Carbon::today())->where('type', 2)->where('owner_id', CNF_OWNER)->get();
    ...
    {
        $dates = Tourbounddates::all();

        $dates = Tourbounddates::select('start')->get();
