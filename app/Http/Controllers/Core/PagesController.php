    ...
    public function getUpdate(Request $request, $id = null)
    {
        ...
        if ($row) {
            ...
            if ('' == $id) {
                ...
                return Redirect::to('core/pages')->with('messagetext', \Lang::get('core.note_restric'))->with('msgstatus', 'error');
            }
        }
        ...
        $banner_position               = \DB::table('banners')->select('position_name')->where('owner_id', '=', CNF_OWNER)->where('position_name', '!=', 'home')->groupBy('position_name')->orderBy('position_name')->get();
        $this->data['banner_position'] = $banner_position;
