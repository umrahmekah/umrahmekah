...
class FormsController extends Controller
{
    ...
    public function getConfiguration(Request $request, $ids)
    {
    ...
    public function getInput(Request $request, $id)
    {
        $f = [
    ...
    public function postField(Request $request, $formID)
    {
        ...
        \DB::table('tb_forms')->where('formID', '=', $formID)->update(['configuration' => json_encode($forms)]);

        return json_encode(['status' => 'success']);
    }

    public function getRemovefield(Request $request, $formID, $field)
    {
        $row = \DB::table('tb_forms')->where('formID', $formID)
                                ->get();
        if (count($row) <= 0) {
    ...
    public function postReorder(Request $request, $ids)
    {
        $row           = $this->model->find($ids);
        $configuration = json_decode($row->configuration, true);
        $i             = 0;
        ...
        foreach ($configuration as $rows) {
            ...
            if (isset($_POST['sortlist'][$rows['field']])) {
                $orders  = ['sortlist' => $_POST['sortlist'][$rows['field']]];
        ...
        //echo '<pre>'; print_r($forms); echo '</pre>'; exit;
        \DB::table('tb_forms')->where('formID', '=', $ids)->update(['configuration' => json_encode($forms)]);

        return json_encode(['status' => 'success']);
    }

    public function convertForm($id, $rows, $num)
    {
        $type     = $rows['type'];
        $field    = $rows['field'];
        ...
        switch ($type) {
            default:
				...
				<span class="pull-right"><a href="#" onclick="MmbModal(\'' . $edit . '\')"><i class="fa fa-pencil fa-lg "></i> Edit </a> | ' . $remove . '</span>
				</div>';
                break;
            ...
            case 'hidden':
				...
				<input type="hidden" name="sortlist[' . $field . ']" class="reorder" value="' . $num . '"  />
				<span class="pull-right"><a href="#" onclick="MmbModal(\'' . $edit . '\')"><i class="fa fa-pencil fa-lg "></i> Edit </a> | 
				' . $remove . '</span>
				</div>';
                break;
    ...
    public function postProccess(Request $request, $formID)
    {
        ...
        if ($validator->passes()) {
            ...
            } elseif ('eav' == $row->method) {
                // DO insert into EAV table
            } elseif ('email' == $row->method) {
